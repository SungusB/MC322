...
public abstract class FaseDeCombate implements Fase {
    private boolean salvarSolicitado = false;
    public boolean isSalvarSolicitado() { return salvarSolicitado; }
    ...
    public void executarCombate() {
        ...
        for (Monstro monstro : new ArrayList<>(monstros)) {
            ...
            if (!monstro.estaVivo()) {
                ...
                if (!continuar) {
                    if (com.rpg.menu.MenuPosTurno.foiSalvarSolicitado()) {
                        this.salvarSolicitado = true;
                    }
                    return;
                }
            }
